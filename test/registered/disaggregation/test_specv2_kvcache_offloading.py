...
class TestReleaseFinishedReq(unittest.TestCase):
    ...
    def test_radix_owned_prefix_is_not_returned_to_allocator(self):
        ...
        self.assertEqual(len(freed), 1)
        self.assertTrue(
            torch.equal(freed[0], torch.arange(12, 20, dtype=torch.int64))
        )
        manager.tree_cache.dec_lock_ref.assert_called_once_with(last_node)
    ...
    def test_partially_protected_prefill_only_frees_unowned_ranges(self):
        ...
        self.assertEqual(len(freed), 2)
        self.assertTrue(
            torch.equal(freed[0], torch.arange(4, 8, dtype=torch.int64))
        )
        self.assertTrue(
            torch.equal(freed[1], torch.arange(8, 20, dtype=torch.int64))
        )
